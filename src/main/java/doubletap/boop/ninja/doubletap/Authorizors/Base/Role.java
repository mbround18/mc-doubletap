...
public class Role {
    ...
    private Policy[] filterPoliciesByEffect(String effect) {
        ...
        Stream<Policy> filteredPolicyStream = policyStream.filter(policy -> policy.effect.toLowerCase(Locale.ROOT).equals(effectLower));
        return filteredPolicyStream.toArray(Policy[]::new);
    }

    public boolean isBaseNode(String node) {
        return this.baseNodes.stream().filter(baseNode -> Pattern.compile(baseNode).matcher(node).matches()).toArray().length > 0;
    }

    public boolean isAllowed(String node) {
        boolean allowedNodesCheck = Arrays.stream(allowedPolicies()).filter(policy -> policy.matchesAttribute(node)).toArray().length > 0;
        if (allowedNodesCheck) {
            boolean deniedNodesCheck = Arrays.stream(deniedPolicies()).filter(policy -> policy.matchesAttribute(node)).toArray().length > 0;
